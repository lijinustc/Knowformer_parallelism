...
class TransductiveKnowledgeGraph:
    def __init__(self, data_path, hidden_dim):
        ...
        self.pe = self.pe()
    def pe(self):
        edge_index = torch.cat((self.train_triplets, self.valid_triplets, self.test_triplets), dim=0)
        edge_index = torch.unique(edge_index, dim=0)
        edge_index = edge_index[:,[0,2]]
        edge_index = edge_index.T
        data = Data(x = torch.zeros(self.num_entity, self.hidden_dim), edge_index = edge_index)
        transform = AddLaplacianEigenvectorPE(k = self.hidden_dim, attr_name = 'pe' , is_undirected= False)
        data = transform(data)
        return data.pe
